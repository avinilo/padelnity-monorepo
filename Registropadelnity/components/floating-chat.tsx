...
const ChatHeader = ({ onClose, isOnline }: { onClose: () => void; isOnline: boolean }) => (
...
const ScrollToBottomButton = ({ onClick, show, unreadCount }: { onClick: () => void; show: boolean; unreadCount: number }) => (
  <div
...
export default function FloatingChat({ context = "auth" }: FloatingChatProps) {
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      ...
      const botMessage: Message = {
        ...
        content: data.response || "Lo siento, no pude procesar tu mensaje. ¿Podrías intentarlo de nuevo?",
        isBot: true,
        timestamp: new Date()
      }
      ...
      const errorMessage: Message = {
        id: (Date.now() + 1).toString(),
        content: "Lo siento, hubo un problema con la conexión. Por favor, verifica tu conexión a internet e inténtalo de nuevo.",
        isBot: true,
        timestamp: new Date()
      }
  ...
  const chatContainerStyle = useMemo(() => {
    const height = viewportHeight || (typeof window !== 'undefined' ? window.innerHeight : 800)
    return {
      height: `${height}px`,
      maxHeight: `${height}px`,
  ...
  return (
    <div className="fixed inset-0 z-50 bg-gradient-to-br from-emerald-500/20 via-emerald-600/20 to-teal-600/20 backdrop-blur-sm">
      <div 
        className="absolute inset-0 flex flex-col animate-in slide-in-from-bottom duration-300"
      ...
      >
        ...
        <div 
          ...
          className="relative z-10 flex-1 overflow-y-auto p-4 space-y-4 scroll-smooth bg-white/5 backdrop-blur-sm"
        ...
        >
          ...
          {isLoading && (
            <div className="flex justify-start mb-4">
              <div className="bg-white/95 backdrop-blur-sm rounded-2xl px-4 py-3 shadow-lg border border-white/20 flex items-center space-x-2">
        ...
        {/* Input Area */}
        <div className="relative z-10 border-t border-white/20 bg-white/95 backdrop-blur-sm p-4">
          {!isOnline && (
            <div className="mb-3 p-2 bg-red-50/90 backdrop-blur-sm border border-red-200 rounded-lg">
              <p className="text-sm text-red-600 text-center">Sin conexión a internet</p>
            </div>
          )}
          ...
          <div className="flex items-end space-x-3">
            <div className="flex-1">
              <Input
                ...
                className="resize-none border-white/30 bg-white/90 backdrop-blur-sm focus:border-emerald-500 focus:ring-emerald-500 rounded-xl shadow-sm"
            ...
            <Button
              ...
              className="w-11 h-11 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 disabled:bg-gray-300 transition-all shadow-lg"
          ...
          {/* Powered by TerretaCode */}
          <div className="mt-3 text-center" style={{ paddingBottom: 'env(safe-area-inset-bottom, 16px)' }}>
            <p className="text-xs text-gray-500">
              Powered by <span className="font-medium text-emerald-600">TerretaCode</span>
            </p>
          </div>
        </div>
      </div>
