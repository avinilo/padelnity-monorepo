...
function ResetPasswordContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (emailError) {
      error("Ingresa tu email para continuar")
      return
    }
    ...
    try {
      await executeWithLoading(async () => {
        ...
        if (resetError) {
          if ((resetError as any)?.message?.includes('rate_limit')) {
            error("Espera unos minutos antes de intentar de nuevo")
          } else {
  ...
  const handleResendEmail = async () => {
    ...
    try {
      await executeWithLoading(async () => {
        const { error: resendError } = await resetPassword(formData.email)
  ...
  const handleOtpSubmit = async () => {
    ...
    try {
      await executeWithLoading(async () => {
        const { error: verifyError } = await verifyOtp(formData.email, otpCode, 'recovery')
        
        if (verifyError) {
          if ((verifyError as any)?.message?.includes('invalid_otp')) {
            error("Verifica el código e inténtalo de nuevo")
          } else if ((verifyError as any)?.message?.includes('expired')) {
            error("Solicita un nuevo código")
          } else {
            error("Inténtalo de nuevo o solicita un nuevo código")
          }
          return
        }

        setShowPasswordForm(true)
        success("Ahora puedes cambiar tu contraseña")
      })
  ...
  if (showPasswordForm) {
    return <PasswordResetWithToken isKeyboardOpen={isKeyboardVisible} />
  }
